...
const ProductDisplay = () => {
  ...
  const cartItemsDisplay = cart.length > 0 ? cart.map(item => {
    const product = products.find(p => p._id === item.productId);
    return (
      <li key={item.productId}>
        {product.name} - Quantity: {item.quantity} - Individual Price: Rs.{product.price * item.quantity}
  ...
  const chunkedProducts = products.length > 0 
      ? chunkArray(products, Math.ceil(products.length / 3)) 
      : [];

  // JSX for displaying products in three rows
  const productRows = products.length > 0 ? chunkedProducts.map((row, index) => (
